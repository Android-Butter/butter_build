...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${BUTTER_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    BUTTER_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/butter/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BUTTER model name
            lunch butter_$target-userdebug
        fi
    fi
    return $?
}
